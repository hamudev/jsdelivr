	...
	if (window.location.hash) {
		if ($(window.location.hash).length) {
			...
			$('html, body')
				.stop()
				.animate({
						scrollTop: $(window.location.hash).offset().top - _0x5d3606 - _0x2c8acb - 20,
					},
					500
				)
	...
	$(document).on('click', 'a[href*="#"]', function(_0x5b4bec) {
		...
		if ($(_0x2ecb34).length && $(this).attr('href').split('#')[0] === '') {
			...
			_0x175c27
				...
				.stop()
				.animate({
						scrollTop: $(_0x2ecb34).offset().top - _0x15af35 - _0x175c27 - 20,
					},
					500
				) :
				$('html, body')
...
function timeago() {
	$('[datetime]:not(.timeago)').each(function() {
		...
		_0x2341c5.addClass('timeago') !_0x2341c5.attr('title') && _0x2341c5.attr('title', _0x2a92be)
